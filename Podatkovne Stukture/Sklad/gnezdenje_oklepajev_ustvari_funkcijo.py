...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTEzN30:1meJIf:8mPPCkU1uFDq0tmLWY9xZlwoIl0'
        try:
            test_data = [
            ...
            Check.equal("oklepaji('3-(2-y)*[1+4**(3+1)]<()>')", True)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTEzOH0:1meJIf:3HMRCEKgcxnTAA3-JAWikQJKQGk'
        try:
            test_data = [
                ("max_globina('burek')", 0),
