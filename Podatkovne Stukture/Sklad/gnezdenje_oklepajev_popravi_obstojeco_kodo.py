...
def oklepaji(niz):
    ...
    for znak in niz:
        if znak in oklep.keys():
            ...
            if sklad.prazen() or oklep[sklad.vrh()] != znak:
                return False
            sklad.odstrani()
    return sklad.prazen()
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE1N30:1nBN1b:k7jFyaVhQox9jHZLHB2Ova-tVT0'
        try:
            test_data = [
                ("oklepaji('(a + b)^2 = (((a^2) + 2ab) + b^2)')", True),
                ("oklepaji('())(()')", False),
                ("oklepaji('[a + b]^2 = (({a^2} + 2ab) + b^2)')", True),
                ("oklepaji('[])({}')", False),
                ('oklepaji("(2 + a) + ((3 - x) + (3 - 2))")', True),
                ('oklepaji("() +krneki-/*+3@ (() - ()((()())))")', True),
                ('oklepaji("[] +krneki-/*+3@ (() - ()(([]{})))")', True),
                ('oklepaji("(((()) + (")', False),
                ('oklepaji("<((()) + ()>")', False),
                ('oklepaji("<({()} + [])>")', True),
                ('oklepaji("(2 + 3) - 3 + ) (")', False),
            ]
            for td in test_data:
                if not Check.equal(*td):
                    break
            
            Check.equal("oklepaji('3-(2-y)*[1+4**(3+1)]<(>')", False)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE1OH0:1nBN1b:OSh_z0d2HArea-7M_r-qoW7KSR4'
        try:
            test_data = [
                ("max_globina('burek')", 0),
                ("max_globina('(a + b)^2 = (((a^2) + 2ab) + b^2)')", 3),
                ("max_globina('[a + b]^2 = (({a^2} + 2ab) + b^2)')", 3),
                ('max_globina("(2 + a) + ((3 - x) + (3 - 2))")', 2),
                ('max_globina("() +krneki-/*+3@ (() - ()((()())))")', 4),
                ('max_globina("[] +krneki-/*+3@ (() - ()(([]{})))")', 4),
                ("max_globina('())(()')", False),
                ("max_globina('[])({}')", False),
                ('max_globina("(((()) + (")', False),
                ('max_globina("<((()) + ()>")', False),
                ('max_globina("(2 + 3) - 3 + ) (")', False),
            ]
            for td in test_data:
                if not Check.equal(*td):
                    break
            
            Check.equal("max_globina('3-(2-y)*[1+4**(3+1)]<(>')", False)
            Check.equal("max_globina('3-(2-y)*[1+4**(3+1)]<({<3x>+2})>')", 4)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
