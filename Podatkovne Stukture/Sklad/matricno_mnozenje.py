...
# Matrično množenje
#
# Naj bo $A$ matrika velikosti $n \times m$ in naj bo $B$ matrika velikosti $p
# \times r$. Če je $m = p$, potem lahko matriki zmnožimo. Matrika $AB$ je
# velikosti $n \times r$. Da jo dobimo potrebujemo $nmr$ množenj. Če je $m \neq
# p$, matrik $A$ in $B$ ne moremo zmnožiti.
# 
# Implementacijo razreda `Sklad` lahko dobite [tukaj](http://zaversnik.fmf.uni-lj.si/sklad.py).
# =====================================================================@029140=
# 1. podnaloga
...
def stevilo_mnozenj(niz, velikosti):
    matrike = Sklad()
    operacij = 0
    for znak in niz:
        if znak == '(':
            ...
            desni = matrike.poberi()
            levi = matrike.poberi()
            (l1, l2) = velikosti[levi]
            (d1, d2) = velikosti[desni]
            if l2 != d1:
                return None
            else:
                operacij += l1 * l2 * d2
                matrike.vstavi(levi)
                velikosti[levi] = (l1, d2)
        else:
            matrike.vstavi(znak)
    return operacij
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE0MH0:1mfkrC:WyNyOmIoq2jlaW_I7XPs5x2iyx0'
        try:
            test_data = [
                ("""stevilo_mnozenj('((AB)C)', {'A': (10, 5), 'B': (5, 20), 'C': (20, 3)})""", 1600),
                ("""stevilo_mnozenj('(A(BC))', {'A': (10, 5), 'B': (5, 20), 'C': (20, 3)})""", 450),
                ("""stevilo_mnozenj('((BA)C)', {'A': (10, 5), 'B': (5, 20), 'C': (20, 3)})""", None),
                ("""stevilo_mnozenj('((((AA)A)A)A)', {'A': (10, 10)})""", 4000),
            ]
            for td in test_data:
                if not Check.equal(*td):
                    break
            
            Check.equal("stevilo_mnozenj('((A((CC)B))((CA)C))', {'A': (13, 13), 'B': (13, 20), 'C': (13, 13)})", None)
            Check.equal("stevilo_mnozenj('(((AB)C)(CD))', {'A': (12, 7), 'B': (7, 19), 'C': (19, 19), 'D': (19,10)})", 11818)
        except:
