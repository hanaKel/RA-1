...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIzNX0:1mqHlo:ESttBKTnoWNEav1pxIudzpp4hbk'
        try:
            # OPTIMALNA REŠITEV
            # def najvecji_podprodukt(sez):
            #     opt = 1
            ...
            #                 opt = prod
            #         else:
            #             # Najbolši produkt do `i` je manjši od 1, torej se ne splača
            #             # začeti prej kot po `i` (saj je produkt 0 elementov enak 1).
            #             prod = 1
            #     return opt
            
            Check.equal('najvecji_podprodukt([0.5, 2, 0.7, 3])', 4.2)
            Check.equal('najvecji_podprodukt([2, 3, 0, 4, 2])', 8)
        except:
