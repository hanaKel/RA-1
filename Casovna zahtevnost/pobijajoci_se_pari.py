...
def nicelni_pari_v_urejenem(sez, i = 0, j = 1):
    dol = len(sez)
    if i == dol or dol == 0:
        return False
    else:
        prvi = sez[i] 
        zadnji = sez[-j]
        if prvi == zadnji == 0 and dol == i + j:
            return False
        elif prvi + zadnji == 0:
            return True
        elif abs(prvi) < zadnji:
            return nicelni_pari_v_urejenem(sez, i, j + 1)
        else:
            return nicelni_pari_v_urejenem(sez, i + 1, j)
...
def nicelni_pari_v_neurejenem(sez):
    return nicelni_pari_v_urejenem(sorted(sez))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIzNn0:1mqHYM:Al9Vj9M5CPOVG3ZgcvVVUjCqo3c'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIzN30:1mqHYM:AtE0yfVlJwyhRVkrbpXg9kFC8UQ'
        try:
