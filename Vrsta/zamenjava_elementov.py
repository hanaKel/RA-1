...
def zamenjaj_elementa(v, k):
    if prestej_elemente(v) < k + 1:
        raise IndexError('vrsta je prekratka')
    elif k == 0:
        raise IndexError('neveljaven indeks: 0')
    else:
        tab = []
        while not v.prazna():
            tab.append(v.zacetek())
            v.odstrani()
        tab[k-1], tab[k] = tab[k], tab[k-1]
        for el in tab:
            v.vstavi(el)
        return v
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE3N30:1nBNvk:EpbGRBwC-xQ8rukzQWsspTses8Q'
        try:
            test_data = [
                (["v = Vrsta([4, 5, 6, 7, 8])",
                  "v_nova = zamenjaj_elementa(v, 3)",
                  "s1 = str(v)",
                  "s2 = str(v_nova)"],
                 {'s1': 'ZACETEK : 4 : 5 : 6 : 7 : 8 : KONEC',
                  's2': 'ZACETEK : 4 : 5 : 7 : 6 : 8 : KONEC'}),
                (["v = Vrsta(['a', 'b', 'd', 'e', 5, 2])",
                  "v_nova = zamenjaj_elementa(v, 2)",
                  "v_nova = zamenjaj_elementa(v_nova, 3)",
                  "s1 = str(v)",
                  "s2 = str(v_nova)"],
                 {'s1': 'ZACETEK : a : b : d : e : 5 : 2 : KONEC',
                  's2': 'ZACETEK : a : d : e : b : 5 : 2 : KONEC'})
            ]
            for td in test_data:
                if not Check.run(*td):
                    break
