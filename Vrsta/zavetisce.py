...
# * `sprejmi(self, ime, vrsta)`, ki evidentira novo žival z imenom `ime`, ki je
#   pripadnik vrste `vrsta` (ta ima lahko vrednost `'pes'` ali `'mačka'`);
...
class Zavetisce:
    ...
    def sprejmi(self, ime, vrsta):
        self.st += 1
        if vrsta == 'mačka':
            self.macke.vstavi((ime, self.st))
        else:
            self.psi.vstavi((ime, self.st))
    def oddaj_psa(self):
        if not self.psi.prazna():
            ime, st = self.psi.zacetek()
            self.psi.odstrani()
            return ime
        else:
            return None
    def oddaj_macko(self):
        if not self.macke.prazna():
            ime, st = self.macke.zacetek()
            self.macke.odstrani()
            return ime
        else:
            return None
    ...
    def oddaj_zival(self):
        '''V posvojitev gre prva v vrsti, če primerjam mačko in psa'''
        if self.psi.zacetek()[1] < self.macke.zacetek()[1]:  
            ime, st = self.psi.zacetek()
            self.psi.odstrani()
            return ime # pes gre v posvojitev
        elif self.psi.zacetek()[1] > self.macke.zacetek()[1]:
            ime, st = self.macke.zacetek()
            self.macke.odstrani()
            return ime # mačka gre v posvojitevv
        else:
            return None # ni ustrezne živali
...
class ZavetiscePoPredpisih:
    def __init__(self):
        self.sez = Vrsta()
    def sprejmi(self, ime, vrsta):
        self.sez.vstavi((ime, vrsta))

    def oddaj_psa(self):
        pom = Vrsta()
        prvi = True
        while not self.sez.prazna():
            ime, zival = self.sez.zacetek()
            if zival == 'pes' and prvi:
                self.sez.odstrani()
                prvi = False
                rezultat = ime
            else:
                pom.vstavi((ime, zival))
                self.sez.odstrani()
        while not pom.prazna():
            ime, zival = pom.zacetek()
            self.sez.vstavi((ime, zival))
            pom.odstrani()
        if prvi:
            return None
        else:
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2Nn0:1miEMU:lS6glW39egCjVLzKzgMU9Jy-zPI'
        try:
            Check.run(["z = Zavetisce()",
                       "l = []",
                       "z.sprejmi('Taček', 'pes')",
                       "z.sprejmi('Šarki', 'pes')",
                       "l.append(z.oddaj_macko())",
                       "z.sprejmi('Tom', 'mačka')",
                       "l.append(z.oddaj_psa())",
                       "z.sprejmi('Nyan', 'mačka')",
                       "l.append(z.oddaj_zival())",
                       "l.append(z.oddaj_macko())"],
                      {'l': [None, 'Taček', 'Šarki', 'Tom']})
            #    ("""macke_psi([('Šarki', 'p'), ('Tom', 'm'), ('Suki', 'm'), ('Taček', 'p'), ('Nyan', 'm'), ('Rex', 'p'), ('Čupko', 'p'), ('Snoopy', 'p'), ('Tiger', 'm')])""",
            #     (['Šarki', 'Taček', 'Rex', 'Čupko', 'Snoopy'], ['Tom', 'Suki', 'Nyan', 'Tiger'])),
            #    ("""macke_psi([('Šarki', 'p'), ('Taček', 'p'), ('Rex', 'p'), ('Čupko', 'p'), ('Snoopy', 'p')])""",
            #     (['Šarki', 'Taček', 'Rex', 'Čupko', 'Snoopy'], [])),
            #    ("""macke_psi([('Tom', 'm'), ('Suki', 'm'), ('Nyan', 'm'), ('Tiger', 'm')])""",
            #     ([], ['Tom', 'Suki', 'Nyan', 'Tiger'])),
        except:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2N30:1miEMU:BMTGeZibsJC3plmqfXz2dZrQSCk'
        try:
            Check.run(["z = ZavetiscePoPredpisih()",
                       "l = []",
                       "z.sprejmi('Taček', 'pes')",
                       "z.sprejmi('Šarki', 'pes')",
                       "l.append(z.oddaj_macko())",
