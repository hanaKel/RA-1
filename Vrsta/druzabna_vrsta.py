...
class DruzabnaVrsta():
    def __init__(self):
        self.vrsta = Vrsta()
    def ime_prvega(self):
        ime, skupina = self.vrsta.zacetek()
        return ime
    def postrezi(self):
        self.vrsta.odstrani()
    def dodaj_prisleka(self, ime, skupina):
            ...
            while not pom.prazna():
                if naj == j:
                    ...
                    ime1, sku = pom.zacetek()
                    self.vrsta.vstavi((ime1, sku))
                    pom.odstrani()
                    j += 1
            if naj == -1:
                self.vrsta.vstavi((ime, skupina))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2NX0:1mi0FP:mmvHIwgM21-3hAR44Fpqg2owgQs'
        try:
            Check.run(["menza = DruzabnaVrsta()",
                       "l = []",
                       "menza.dodaj_prisleka('Marjan', 'FMF')",
                       "menza.dodaj_prisleka('Petra', 'FKKT')",
                       "menza.dodaj_prisleka('Borut', 'FDV')",
                       "menza.dodaj_prisleka('Tanja', 'FMF')",
                       "menza.dodaj_prisleka('Boris', 'FKKT')",
                       "l.append(menza.ime_prvega())",
                       "menza.postrezi()",
                       "l.append(menza.ime_prvega())",
                       "menza.postrezi()",
                       "l.append(menza.ime_prvega())",
                       "menza.postrezi()",
                       "menza.dodaj_prisleka('Simon', 'FMF')",
                       "l.append(menza.ime_prvega())",
                       "menza.postrezi()",
                       "l.append(menza.ime_prvega())",
                       "menza.postrezi()",
                       "l.append(menza.ime_prvega())",
                       "menza.postrezi()"],
                      {'l': ['Marjan', 'Tanja', 'Petra', 'Boris', 'Borut', 'Simon']})
        except:
