...
# Funkcija naj iz vrste `v` odstrani $n$-ti zaporedni element (šteti začnemo
# pri 0). Če vrsta ne vsebuje $n$-tega elementa (ker je prekratka), naj je
...
# ([implementacija](http://zaversnik.fmf.uni-lj.si/sklad.py)).
# 
# Zgled:
# 
#     >>> v = Vrsta([1, 2, 3, 4, 5])
...
def obrni(v):
    ...
    while not v.prazna():
        ...
        v.odstrani()
        #stevec += 1 if v.zacetek == pomozni.vrh(): stevec = 0
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2MH0:1mhzxT:hk2c2FIj8VCEdHEuXPFMs1cJK7Y'
        try:
            ...
            # n = randrange(400, 550)
            # Check.secret(prestej_elemente(Vrsta([i%40 for i in range(n)])))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2MX0:1mhzxT:0jzlwEjBplvW1nWcDJ8jpGkGoFM'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2Mn0:1mhzxT:vybQ4Rq_zYic-mcN5Z1CJUDzX84'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "odstrani_ntega(v, 3)",
                       "potem1 = str(v)",
            ...
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       ...
                       "odstrani_ntega(v,1)",
                       "potem = str(v)"],
                      {'potem': 'ZACETEK : 1 : 3 : 4 : 5 : 6 : 7 : 8 : KONEC'})
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2M30:1mhzxT:zCqcJDG5B9bOeQ04sA-ymqwne60'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "odstrani_pogojno(v, lambda x: x % 2 == 0)",
                       "potem = str(v)"],
            ...
            Check.run(["v = Vrsta([1, 2, 7, 4, 5, 4, 7, 8])",
                       "odstrani_pogojno(v, lambda x: x % 3 == 2)",
                       "potem = str(v)"],
                      {'potem': 'ZACETEK : 1 : 7 : 4 : 4 : 7 : KONEC'})
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2NH0:1mhzxT:uG0omUe_jAQYnmbW-gMrKqAtobo'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "obrni(v)",
                       "potem = str(v)"],
                      {'potem': 'ZACETEK : 8 : 7 : 6 : 5 : 4 : 3 : 2 : 1 : KONEC'})
            Check.run(["v = Vrsta([i for i in range(52)] + [24, 13, 'a', 3]" +
                       "+ [i for i in range(220,250)])",
                       "obrni(v)",
                       "potem = str(v)"],
                      ({'potem': 'ZACETEK : ' +
                        ' : '.join(reversed([str(i) for i in range(220,250)])) +
                        ' : 3 : a : 13 : 24 : ' +
                        ' : '.join(reversed([str(i) for i in range(52)])) +
                        ' : KONEC'}))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
