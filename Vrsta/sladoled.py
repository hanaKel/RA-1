...
#     KIOSK 3 2 3 1 3 2 1
# 
# Otrok, ki je zdaj na začetku vrste, kupi sladoled in gre, v vrsti pa
# ostanejo:
# 
#     KIOSK 2 3 1 3 2 1
# 
...
# naj $i$-to število v izhodnem seznamu pomeni, kateri po vrsti kupi sladoled
# otrok, ki je bil na začetku $i$-ti v vrsti. Zgled:
# 
...
def naj_moc_fun(vrsta):
    ''' Funkcija vrne največje število v vrsti.'''
    naj = 0
    pom = Vrsta()
...
def kdaj_pridem_na_vrsto(sez):
    vrsta = Vrsta()
    for i, st in enumerate(sez):
        vrsta.vstavi((i, st))
    naj_moc = naj_moc_fun(vrsta)
    kateri = 1
    while not vrsta.prazna():
        i, st = vrsta.zacetek()
        if st == naj_moc:
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTE2OH0:1miEOm:YUS433rKux0dOf7HcmJ3MQ7q95E'
        try:
            Check.equal("kdaj_pridem_na_vrsto([2, 1, 3, 2, 3, 1, 3])", [4, 7, 1, 5, 2, 6, 3])
        except:
