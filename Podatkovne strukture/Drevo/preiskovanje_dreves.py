...
def vsota(drevo):
    if drevo == Drevo() or drevo.prazno:
        return 0
    elif drevo.levo == Drevo() and drevo.desno == Drevo():
        return drevo.podatek
    elif drevo.levo == Drevo():
        return drevo.podatek + vsota(drevo.desno)
    elif drevo.desno == Drevo():
        return drevo.podatek + vsota(drevo.levo)
    else: # ima levo in desno poddrevo
        return vsota(drevo.levo) + vsota(drevo.desno) + drevo.podatek
# =====================================================================@029248=
# 2. podnaloga
...
def je_list(dr):
    if dr == Drevo() or dr.prazno:
        return None
    elif dr.levo == Drevo() and dr.desno == Drevo():
        return True
    else:
        return False
...
def minimum(dr, mini = None):
    if dr == Drevo():
        return None
    if mini == None:
        mini = dr.podatek
    else:
        mini = min(dr.podatek, mini)
    if dr.levo == Drevo() and dr.desno == Drevo():
        return mini
    elif dr.levo == Drevo():
        desna = minimum(dr.desno, mini)
        return min(mini, desna)
    elif dr.desno == Drevo():
        leva = minimum(dr.levo, mini)
        return min(mini, leva)
    else: # ima levo in desno poddrevo
        return min(minimum(dr.levo, mini), minimum(dr.desno, mini))
...
def je_list(dr):
    if dr == Drevo() or dr.prazno:
        ...
        return False
def premer(drevo, koren = True):
    if drevo.prazno:
        return float('-inf')
    if je_list(drevo):
        return 0
    cez_koren = visina(drevo.levo) + visina(drevo.desno) 
    brez_korena = max(premer(drevo.levo, koren = False), premer(drevo.desno, koren = False))
    if (drevo.levo == Drevo() or drevo.desno == Drevo()) and koren:
        cez_koren -= 1
    return max(cez_koren, brez_korena, 1) 
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTI0N30:1msANs:QiwP2MHICGo-Ys4iKGcr-OJp59Q'
        try:
            test_data = [
                ('vsota(Drevo())', 0),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTI0OH0:1msANs:fXPi21vsc_FkcFpUZ3oTXbsfVvc'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTI0OX0:1msANs:81sXdy7V27eWYxQoL20Veie4kEk'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTI1MH0:1msANs:7XpdOZctNllqegYQ-CW3sfaEqlk'
        try:
            test_data = [
