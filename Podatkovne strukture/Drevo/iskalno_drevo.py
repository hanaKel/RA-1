...
class IskalnoDrevo:
    ...
    def dodaj(self, novi):
        '''
            v iskalno drevo na pravilno mesto vstavi podatek
        '''
        if self.prazno:
            self.podatej = novi
        else:
            if novi > self.podatek:
                self.desno.dodaj(novi)
            else:
                self.levo.dodaj(novi)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTI3Nn0:1muzJc:WhZF4inJPppGuymRTAvMTT0wgJo'
        try:
            test_data_run = [
                (["d = IskalnoDrevo()",
                  "d.dodaj(3)",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTI3N30:1muzJc:frsXCrOs-w0kDefLq_NvWzam5Tw'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTI3OH0:1muzJc:gzqh5SlwAxUURWhm3hTttxmHrIg'
        try:
            test_data = [
