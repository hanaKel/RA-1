...
# =============================================================================
from veriga_vozlov import Vozel, dodaj_na_konec, dodaj_na_zacetek, vrni_seznam, iz_seznama
from sestavljanje_verig_vozlov import sodi_in_lihi
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxNH0:1nBiGR:B60dS3rP3_zr91-r87UA2SpjJ7o'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 1, 3, 4, 9, 8])",
                  "v = preuredi_parnost(v)",
                  "l = vrni_seznam(v)"],
                 {'l': [7, 5, 1, 3, 9, 2, 4, 8]}),
                (["v = iz_seznama([i for i in range(108)])",
                  "v = preuredi_parnost(v)",
                  "l = vrni_seznam(v)"],
                 {'l': [2*i+1 for i in range(54)] + [2*i for i in range(54)]})
            ]
            for td in test_data:
                if not Check.run(*td):
                    break
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxNX0:1nBiGR:EcHlv2E_gszXLbAttjK-tRggKWo'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 1])",
                  "v = obrni_na_mestu(v)",
                  "l = vrni_seznam(v)"],
                 {'l': [1, 2, 5, 7]}),
                (["v = iz_seznama([i for i in range(51)])",
                  "v = obrni_na_mestu(v)",
                  "l = vrni_seznam(v)"],
                 {'l': [50-i for i in range(51)]}),
            ]
            for td in test_data:
                if not Check.run(*td):
                    break
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxNn0:1nBiGR:oYncbdFUoNRtPfcS_IgjXpKp9RY'
        try:
