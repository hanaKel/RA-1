...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxN30:1nBhrX:ImDT4jKR8bBksXN58cVNyNm0tIs'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 1, 3, 4, 9, 8])",
                  "v = preuredi_parnost(v)",
                  "l = vrni_seznam(v)"],
                 {'l': [7, 5, 1, 3, 9, 2, 4, 8]}),
                (["v = iz_seznama([i for i in range(154)])",
                  "v = preuredi_parnost(v)",
                  "l = vrni_seznam(v)"],
                 {'l': [2*i+1 for i in range(77)] + [2*i for i in range(77)]})
            ]
            for td in test_data:
                if not Check.run(*td):
                    break
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxOH0:1nBhrX:Z8JBpF9yE1r1UBWiPo6ehlISqFY'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 1])",
                  "v = obrni_na_mestu(v)",
                  "l = vrni_seznam(v)"],
                 {'l': [1, 2, 5, 7]}),
                (["v = iz_seznama([i for i in range(60)])",
                  "v = obrni_na_mestu(v)",
                  "l = vrni_seznam(v)"],
                 {'l': [59-i for i in range(60)]}),
            ]
            for td in test_data:
                if not Check.run(*td):
                    break
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxOX0:1nBhrX:OuR9MuUdGob9L4KgsCT7w5EhBvg'
        try:
