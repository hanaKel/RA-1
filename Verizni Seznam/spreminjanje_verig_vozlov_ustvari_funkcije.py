...
def vstavi_na_mesto(prvi,n,x):
    ...
    if n > len(tabela):
        while st < n:
            tabela.insert(st,None)
            st+=1
        
    nov = None
    for i in tabela[::-1]:
        nov = dodaj_na_zacetek(nov,i)
    return nov

# če je n večji od len verige bi mogla vmesne člene do n nastavt na none
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxMH0:1nBgGh:p9bSAVZy1IdqMYljYLOG2vVwAvQ'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxMX0:1nBgGh:ulhdlRibBZiVJ-S2tCo0lU-rGdg'
        try:
            test_data = [
                (["v = iz_seznama([1, 2, 4, 7, 8])",
                  "v = vstavi_v_urejenega(v, 3)",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxMn0:1nBgGh:iql2_MZkNlpXZ2EjfMwY6ejKOns'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 5, 3, 5])",
                  "v = odstrani(v, 5)",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2ODQ4LCJwYXJ0IjoyOTIxM30:1nBgGh:OAGMk8MydcKgAQXiMIHyZSCWrmU'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2])",
                  "v = multipliciraj_verigo(v, 3)",
